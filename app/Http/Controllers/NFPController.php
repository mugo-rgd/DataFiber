<?php
// app/Http/Controllers/NFPController.php
...
class NFPControllerx extends Controller
{
    ...
    {
        $returns = NFPComplianceReturn::with('submitter')
        ...
        return view('admin.nfp.index', compact('returns'));
    }

    public function create()
    {
        return view('admin.nfp.create');
    }
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'tariff_structure' => 'nullable|file|mimes:pdf,xlsx,xls|max:5120',
            'company_stamp' => 'nullable|file|mimes:jpg,jpeg,png,pdf|max:2048',
        ]);

        // Handle file uploads
        $documents = [];
        $documentFields = ['shareholding_cert', 'audited_financials', 'tax_compliance', 'tariff_structure'];

        foreach ($documentFields as $field) {
            if ($request->hasFile($field)) {
                $path = $request->file($field)->store('nfp-documents/' . date('Y'), 'public');
        ...
        $nfpReturn = NFPComplianceReturn::create([
            ...
            'primary_numbers' => $request->primary_numbers,
            'secondary_numbers' => $request->secondary_numbers,
            'bulk_sms' => $request->bulk_sms,
            'broadband_infrastructure' => $request->broadband,
            'staff_data' => $request->staff,
            ...
            'compliance_id' => $status === 'submitted' ? $this->generateComplianceId('NFP') : null,
            'tracking_code' => $status === 'submitted' ? $this->generateTrackingCode() : null,
        ]);

        $message = $status === 'draft' ? 'Draft saved successfully!' : 'NFP Compliance Return submitted successfully!';

        return redirect()->route('nfp.index')->with('success', $message);
    }

    public function show($id)
    {
        $return = NFPComplianceReturn::with(['submitter', 'approver'])->findOrFail($id);

        if (auth()->user()->role !== 'admin' && $return->submitted_by !== auth()->id()) {
            abort(403);
        }

        return view('admin.nfp.show', compact('return'));
    }

    public function edit($id)
    {
        $return = NFPComplianceReturn::findOrFail($id);
        ...
        if ($return->status === 'approved') {
            return redirect()->route('nfp.index')->with('error', 'Approved returns cannot be edited.');
        }

        return view('admin.nfp.edit', compact('return'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('submit')) {
            $return->update([
                'status' => 'submitted',
                'submitted_at' => now(),
                'compliance_id' => $this->generateComplianceId('NFP'),
                'tracking_code' => $this->generateTrackingCode(),
            ]);
        }

        return redirect()->route('nfp.index')->with('success', 'Return updated successfully!');
    }
    ...
    public function approve(Request $request, $id)
    {
        ...
        if ($request->official_decision === 'approved') {
            $updateData['certificate_number'] = $this->generateCertificateNumber('NFP');
            $updateData['certificate_valid_until'] = now()->addYear();
        }

        $return->update($updateData);

        return redirect()->route('nfp.show', $id)->with('success', 'Return ' . $request->official_decision . ' successfully!');
    }
