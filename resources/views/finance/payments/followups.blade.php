...
@section('title', 'Payment Followups - Finance')
            ...
            <div class="card shadow">
                <div class="card-body">
                    <form method="GET" class="row">
                        ...
                        <div class="col-md-4">
                            <label>&nbsp;</label>
                            <a href="{{ route('finance.payments.followups') }}" class="btn btn-secondary btn-block">Reset Filters</a>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr class="{{ $payment->due_date < now() && $payment->status != 'paid' ? 'table-danger' : ($payment->due_date->diffInDays(now()) <= 3 && $payment->status != 'paid' ? 'table-warning' : '') }}">
                                    <td>
                                        <strong>{{ $payment->customer->name ?? 'N/A' }}</strong>
                                        <br>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        <form action="{{ route('finance.payments.destroy', $payment->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this followup?');">
                                            @csrf
                                            @method('DELETE')
                    ...
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            <div class="text-xs font-weight-bold text-danger text-uppercase mb-1">
                                Overdue Amount</div>
                            <div class="h5 mb-0 font-weight-bold text-gray-800">
                                KES {{ number_format($followups->where('status', 'overdue')->sum('amount'), 2) }}
                            </div>
                        </div>
                        <div class="col-auto">
                        ...
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800">
                                {{ $followups->where('status', 'pending')->filter(function($payment) {
                                    return $payment->due_date->diffInDays(now()) <= 3 && $payment->due_date >= now();
                                })->count() }}
                            </div>
                        </div>
                        <div class="col-auto">
