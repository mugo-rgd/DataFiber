...
function openConversation(conversationId) {
    ...
    document.getElementById('messages').innerHTML = `
        <div class="text-center text-muted py-5">
            <div class="spinner-border text-kp-blue mb-3" role="status">
    ...
    .catch(error => {
        ...
        document.getElementById('messages').innerHTML = `
            <div class="text-center text-muted py-5">
                ...
                <button class="btn btn-sm btn-kp-primary" onclick="openConversation(${conversationId})">
                    Try Again
                </button>
            </div>
        `;
...
function renderMessages(messages) {
    ...
    container.innerHTML = sortedMessages.map(msg => `
        <div class="message mb-3 ${msg.user_id === currentUser.id ? 'text-end' : 'text-start'}" data-id="${msg.id}">
            <div class="d-inline-block ${msg.user_id === currentUser.id ? 'bg-kp-blue text-white' : 'bg-light'}"
...
function addMessageToUI(message) {
    ...
    const messageHtml = `
        <div class="message mb-3 text-end" data-id="${message.id}">
            <div class="d-inline-block bg-kp-blue text-white"
