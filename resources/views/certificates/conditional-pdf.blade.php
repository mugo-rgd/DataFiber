    ...
    <div class="certificate-container">
        ...
        <div class="main-content keep-together">
            ...
            <div class="section">
                <table>
                    <tr>
                        <th style="width: 50%;">Certificate Number:</th>
    ...
    <table class="signature-table">
        <tr>
            ...
            <td class="signature-cell">
                ...
                <div class="signature-image-container">
                    @if($conditionalCertificate->engineer_signature_path)
                        @php
                            $path = $conditionalCertificate->engineer_signature_path;
                            $directPath = storage_path('app/public/' . $path);

                            if (file_exists($directPath)) {
                                $signatureData = base64_encode(file_get_contents($directPath));
                                $signatureSrc = "data:image/png;base64,{$signatureData}";
                            } elseif (file_exists(public_path('storage/' . $path))) {
                                $signatureSrc = asset('storage/' . $path);
                            } else {
                                $signatureSrc = null;
                            }
                        @endphp
            ...
            <td class="signature-cell">
                ...
                <div class="signature-details">
                    <p><strong>Name:</strong> {{ $conditionalCertificate->lessee }}</p>
                    <p><strong>Designation:</strong> {{ $conditionalCertificate->lessee_designation }}</p>
                    <p><strong>Date:</strong> {{ $conditionalCertificate->certificate_issue_date ? date('d/m/Y', strtotime($conditionalCertificate->certificate_issue_date)) : '' }}</p>
                </div>

                <div class="signature-image-container">
                    {{-- Empty space for lessee signature --}}
                </div>
            ...
            <div class="dates-section keep-together">
                <p class="text-bold">This Certificate is dated the: {{ date('d/m/Y', strtotime($conditionalCertificate->certificate_issue_date ?? $conditionalCertificate->certificate_date)) }}</p>
