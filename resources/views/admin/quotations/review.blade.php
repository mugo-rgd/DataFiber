...
<div class="container-fluid">
            ...
            <div class="d-flex justify-content-between align-items-center flex-wrap">
                <div>
                    <h1 class="h3 text-gray-800">
                        ...
                        Review & Revise Quotation
                    </h1>

                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a href="{{ route('admin.quotations.index') }}">Quotations</a>
                            </li>
                ...
                <div class="mt-2 mt-sm-0">
                    <span class="badge bg-danger p-2">
                        ...
                        Status: {{ ucfirst($quotation->status ?? 'Rejected') }}
                    </span>

                    <span class="badge bg-kp-blue p-2 ms-2">
            ...
            <div class="alert alert-danger border-left-3">
                <div class="d-flex">
                    ...
                    <div class="flex-grow-1 ms-3">
                        ...
                        @if($quotation->rejection_reason ?? false)
                            <p class="mb-0">
                        ...
                        @if($quotation->rejection_notes ?? false)
                            <p class="mb-0 small text-muted">
                                <strong>Notes:</strong>
                                {{ $quotation->rejection_notes }}
                            </p>
                        @endif

                        <p class="mb-0 mt-2 small">
                            Review the quotation details below, update the pricing/services/routes, then save or resend.
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-header bg-kp-blue text-white d-flex justify-content-between align-items-center cursor-pointer"
                     data-bs-toggle="collapse"
                     data-bs-target="#designRequestInfo">
                    <h5 class="mb-0">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-2">
                                    ...
                                    <span>{{ $designRequest->customer->name ?? 'N/A' }}</span>
                                </div>

                                <div class="mb-2">
                                    <strong class="text-muted">Title:</strong>
                                    <span>{{ $designRequest->title ?? 'N/A' }}</span>
                                </div>

                                <div class="mb-2">
                                    <strong class="text-muted">Requested:</strong>
                                    <span>
                                        {{ optional($designRequest->created_at)->format('M d, Y') ?? 'N/A' }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-2">
                                    ...
                                    <p class="mb-0">
                                        {{ $designRequest->description ?? 'N/A' }}
                                    </p>
                                </div>

                                @if($designRequest->technical_requirements ?? false)
                                    <div class="mt-2">
                                        <strong class="text-muted">Technical Requirements:</strong>
                                        <p class="mb-0 text-muted">
                                            {{ $designRequest->technical_requirements }}
                                        </p>
                                    </div>
                                @endif
                            </div>
                ...
                <div class="card-body p-3">
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <div>{{ optional($quotation->created_at)->format('M d, Y') ?? 'N/A' }}</div>
                        </div>

                        <div class="col-md-3">
                            <small class="text-muted">Valid Until:</small>
                            <div>
                                {{ $quotation->valid_until ? \Carbon\Carbon::parse($quotation->valid_until)->format('M d, Y') : 'N/A' }}
                            </div>
                        </div>

                        <div class="col-md-3">
                            <small class="text-muted">Tax Rate:</small>
                            <div>{{ number_format(($quotation->tax_rate ?? 0) * 100, 2) }}%</div>
                        </div>

                        <div class="col-md-3">
                            <small class="text-muted">Previous Subtotal:</small>
                            <div>
                                {{ $quotation->currency ?? 'USD' }}
                    ...
                    @if($quotation->customer_notes ?? false)
                        <div class="mt-2 pt-2 border-top">
                            <small class="text-muted">Previous Customer Notes:</small>
                            <p class="mb-0 small">
    ...
    <form action="{{ route('admin.quotations.update', $quotation) }}" method="POST" id="quotationForm">
        ...
        <input type="hidden" name="parent_quotation_id" value="{{ $quotation->id }}">
        ...
        <div class="row">
                    ...
                    <div class="card-header bg-kp-blue text-white d-flex justify-content-between align-items-center py-2">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <span class="badge bg-light text-dark">
                                {{ $commercialRoutes->flatten()->count() }} routes
                            </span>
                        </div>
                    </div>
                                ...
                                @foreach($groupedRoutes as $groupName => $routes)
                                    <div class="mb-3 route-group border rounded">
                                        <div class="route-group-header bg-light p-2 rounded d-flex justify-content-between align-items-center cursor-pointer">
                                            ...
                                            <button type="button"
                                                    class="btn btn-sm btn-link group-toggle-btn"
                                                    data-target="group-{{ $loop->index }}">
                                                <i class="fas fa-chevron-down"></i>
                                            </button>
                                        </div>

                                        <div class="route-group-body p-2"
                                             id="group-{{ $loop->index }}"
                                             style="display: none;">
                                            @foreach($routes as $route)
                                ...
                                <div class="row text-center small">
                                    <div class="col-3">
                                        <span class="fw-bold text-kp-blue">
                                            {{ $commercialRoutes->flatten()->count() }}
                                        </span>
                                        <span class="text-muted d-block">Total</span>
                                    </div>

                                    <div class="col-3">
                                        <span class="fw-bold text-kp-yellow">
                                            {{ $commercialRoutes['Premium']->count() ?? 0 }}
                                        </span>
                                    ...
                                    <div class="col-3">
                                        <span class="fw-bold text-kp-blue">
                                            {{ $commercialRoutes['Non Premium']->count() ?? 0 }}
                                        </span>
                                    ...
                                    <div class="col-3">
                                        <span class="fw-bold text-info">
                                            {{ $commercialRoutes['Metro']->count() ?? 0 }}
                                        </span>
                        ...
                        <div class="card shadow-sm mb-4">
                            <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center py-2">
                                ...
                                <button type="button"
                                        ...
                                        data-bs-target="#customRouteModal">
                                                    ...
                                                    <label class="form-check-label w-100" for="custom_route_{{ $route->id }}">
                                                        ...
                                                        <div class="small mt-1">
                                                            <span class="text-success">
                                                                {{ $route->currency ?? 'USD' }}
                                                                {{ number_format($route->monthly_cost, 2) }}/mo
                                                            </span>

                                                            <span class="text-muted ms-2">
                                                                CAPEX:
                                                                {{ $route->currency ?? 'USD' }}
                                                                {{ number_format($route->capital_expenditure, 2) }}
                                                            </span>
                                                        </div>
                                                    </label>
                                                </div>
                                    ...
                                    <div class="p-2 border-bottom bg-light">
                                        <div class="form-check">
                                            <input class="form-check-input"
                                                   type="checkbox"
                                                   id="selectAllColocationServices">
                                        ...
                                        @foreach($groupedServices as $groupName => $services)
                                            <div class="mb-2 service-group border rounded">
                                                <div class="service-group-header bg-light p-2 d-flex justify-content-between align-items-center cursor-pointer">
                                                    ...
                                                    <button type="button"
                                                            class="btn btn-sm btn-link p-0 group-toggle-btn"
                                                            data-target="service-group-{{ $loop->index }}">
                                                        <i class="fas fa-chevron-down small"></i>
                                                    </button>
                                                </div>

                                                <div class="service-group-body p-2"
                                                     id="service-group-{{ $loop->index }}"
                                                     style="display: none;">
                                                    @foreach($services as $service)
                                                        @php
                                                            ...
                                                            $monthlyRateNumeric = $monthlyPriceUsd > 0
                                                                ? $monthlyPriceUsd
                                                                : ($recurrentPerAnnum > 0 ? $recurrentPerAnnum / 12 : 0);

                                                            $setupFeeNumeric = $setupFeeUsd > 0
                                                                ? $setupFeeUsd
                                                                : ($oneoffRate > 0 ? $oneoffRate : 0);
                                                                    ...
                                                                    <label class="form-check-label w-100" for="service_{{ $serviceId }}">
                                                                        <div class="d-flex justify-content-between">
                                                                            ...
                                                                            <span class="text-success small">
                                                                                ${{ number_format($monthlyRateNumeric, 2) }}/mo
                                                                            </span>
                                                                        </div>
                                                                        ...
                                                                        <div class="service-configuration mt-2 p-2 bg-light rounded" style="display: none;">
                                                                            ...
                                                                            <div class="d-flex justify-content-between mt-1 small">
                                                                                <span>Monthly:</span>
                                                                                <strong class="monthly-cost text-success"
                                                                                        data-service-id="{{ $serviceId }}">
                                                                                    ${{ number_format($monthlyRateNumeric, 2) }}
                                                                                </strong>
                                                                            </div>
                                                                            ...
                                                                            <div class="d-flex justify-content-between small">
                                                                                <span>Total:</span>
                                                                                <strong class="total-cost text-kp-blue"
                                                                                        data-service-id="{{ $serviceId }}">
                                                                                    $0.00
                                                                                </strong>
                                                                            </div>
            ...
            <div class="col-lg-4">
                <div class="sticky-top" style="top: 20px;">
                            ...
                            <div class="row mt-2 g-1">
                                <div class="col-7">
                                    ...
                                    <input type="number"
                                           name="tax_rate"
                                           id="tax_rate"
                                           value="{{ old('tax_rate', $quotation->tax_rate ?? 0.16) }}"
                                           min="0"
                                           max="0.5"
                                           step="0.01"
                    ...
                    {{-- Quotation Details --}}
                    <div class="card shadow-sm mb-4">
                        <div class="card-header bg-info text-white py-2 cursor-pointer"
                             data-bs-toggle="collapse"
                             data-bs-target="#quotationDetails">
                            <h6 class="mb-0">
                            ...
                            <div class="card-body p-3">
                                ...
                                <div class="mb-2">
                                    ...
                                    <div class="btn-group btn-group-sm mb-1 w-100">
                                        ...
                                        <button type="button"
                                                ...
                                                data-bs-toggle="modal"
                                                data-bs-target="#scopeTemplatesModal">
                                            Templates
                                        </button>

                                        <button type="button"
                                                class="btn btn-outline-info btn-sm"
                                                onclick="formatScopeOfWork()">
                                            Format
                                        </button>
                                    </div>
                                    ...
                                    <textarea name="scope_of_work"
                                              ...
                                              class="form-control form-control-sm"
                                              rows="3"
                                              required>{{ old('scope_of_work', $quotation->scope_of_work ?? 'SCOPE OF WORK') }}</textarea>
                                ...
                                <div class="mb-2">
                                    ...
                                    <textarea name="terms_and_conditions"
                                              ...
                                              class="form-control form-control-sm"
                                              rows="3">{{ old('terms_and_conditions', $quotation->terms_and_conditions ?? '') }}</textarea>
                                </div>
                                ...
                                <div class="mb-2">
                                    ...
                                    <textarea name="pricing_notes"
                                              ...
                                              class="form-control form-control-sm"
                                              rows="2"
                                              placeholder="Discounts, payment plans...">{{ old('pricing_notes', $quotation->pricing_notes ?? '') }}</textarea>
                                </div>
                                ...
                                <div class="row g-2 mb-2">
                                    <div class="col-6">
                                        ...
                                        <input type="date"
                                               ...
                                               id="valid_until"
                                               class="form-control form-control-sm"
                                               value="{{ old('valid_until', $quotation->valid_until ? \Carbon\Carbon::parse($quotation->valid_until)->format('Y-m-d') : now()->addDays(30)->format('Y-m-d')) }}">
                                    </div>
                                ...
                                <div class="form-check form-switch mb-1">
                                    <input class="form-check-input"
                                           ...
                                           id="include_tax"
                                           checked>
                                    <label class="form-check-label small" for="include_tax">
                                        Include Tax
                                    </label>
                                </div>
                                ...
                                <div class="form-check form-switch mb-2">
                                    <input class="form-check-input"
                                           ...
                                           checked>
                                    <label class="form-check-label small" for="allow_negotiation">
                                        Allow Negotiation
                                    </label>
                                </div>
                                ...
                                <div class="mb-2">
                                    ...
                                    <textarea name="special_instructions"
                                              ...
                                              class="form-control form-control-sm"
                                              rows="2"
                                              placeholder="Internal notes...">{{ old('special_instructions', $quotation->special_instructions ?? '') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card shadow-sm mb-4">
                        ...
                        <div class="card-body p-3">
                            <textarea name="revision_notes"
                                      ...
                                      class="form-control form-control-sm"
                                      rows="3"
                                      placeholder="Explain what changes were made in this revision...">{{ old('revision_notes') }}</textarea>
                            <small class="text-muted">
                                These notes can help explain what changed.
                            </small>
                        </div>
                    </div>
...
{{-- @endsection --}}
<!-- Modals - Copy directly from create blade -->
