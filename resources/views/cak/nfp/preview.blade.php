...
@section('title', 'NFP Compliance Return Preview')
...
<div class="container">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <div>
            <h4 class="mb-0">
                <i class="fas fa-network-wired me-2 text-kp-yellow"></i> NFP Compliance Preview
            </h4>
        ...
        <div>
            <span class="badge
                ...
                <i class="fas
                    ...
                    @elseif($record->status == 'under_review') fa-clock
    ...
    <div class="card shadow-sm mb-3 bg-light">
        <div class="card-body py-2">
            <div class="d-flex justify-content-between align-items-center flex-wrap small">
                ...
                <div class="text-center {{ in_array($record->status, ['submitted', 'generated']) ? 'text-kp-yellow' : ($record->status != 'draft' ? 'text-kp-green' : 'text-muted') }}">
                    <i class="fas fa-file-pdf"></i> Generated
                </div>
                <div class="text-muted">→</div>
                <div class="text-center {{ $record->status == 'submitted' ? 'text-kp-yellow' : ($record->status != 'draft' && $record->status != 'generated' ? 'text-kp-green' : 'text-muted') }}">
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-header bg-white">
            <strong><i class="fas fa-info-circle me-1 text-kp-yellow"></i> Basic Information</strong>
        </div>
        <div class="card-body">
            <div class="row">
    ...
    <!-- NETWORK LOCATION CARD (NFP SPECIFIC) -->
    @if($record->latitude || $record->longitude || $record->fibre_km || $record->tower_count)
    <div class="card shadow-sm mb-3">
        <div class="card-header bg-white">
            <strong><i class="fas fa-map-marker-alt me-1 text-danger"></i> Network Facility Location</strong>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-3 mb-2">
                    <label class="text-muted small">Latitude</label>
                    <div class="fw-bold">{{ $record->latitude ?? '—' }}</div>
                </div>
                <div class="col-md-3 mb-2">
                    <label class="text-muted small">Longitude</label>
                    <div class="fw-bold">{{ $record->longitude ?? '—' }}</div>
                </div>
                <div class="col-md-3 mb-2">
                    <label class="text-muted small">Fibre Optic Cable (km)</label>
                    <div class="fw-bold">{{ number_format($record->fibre_km ?? 0, 2) }} km</div>
                </div>
                <div class="col-md-3 mb-2">
                    <label class="text-muted small">Number of Towers</label>
                    <div class="fw-bold">{{ $record->tower_count ?? 0 }}</div>
                </div>
            </div>
            @if($record->latitude && $record->longitude)
            <div class="mt-2">
                <button type="button" class="btn btn-sm btn-outline-info" onclick="viewNetworkMap({{ $record->id }})">
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body">
            <div class="d-flex flex-wrap gap-2">
                ...
                <!-- PRINT / DOWNLOAD PDF -->
                <a href="{{ route('nfp.print', $record->id) }}" target="_blank" class="btn btn-kp-primary">
                    <i class="fas fa-print me-1"></i> Print / Download PDF
                </a>

                <!-- GENERATE PDF (if generated status) -->
                @if($record->status === 'generated')
                    <form method="POST" action="{{ route('nfp.generate-pdf', $record->id) }}" class="d-inline">
                        @csrf
                        <button class="btn btn-secondary">
                            <i class="fas fa-sync-alt me-1"></i> Regenerate PDF
                        </button>
                    </form>
                @endif

                <!-- EDIT (only for draft, generated, or rejected) -->
                @if(in_array($record->status, ['draft', 'generated', 'rejected']))
                    <a href="{{ route('nfp.edit', $record->id) }}" class="btn btn-kp-warning">
                ...
                @if($record->status === 'draft')
                    <form method="POST" action="{{ route('nfp.submit', $record->id) }}" class="d-inline"
                          onsubmit="return confirm('Submit this NFP return for processing?\n\nYou will be able to generate a PDF after submission.')">
                        @csrf
                ...
                @if($record->status === 'submitted')
                    <form method="POST" action="{{ route('nfp.submit-to-cak', $record->id) }}" class="d-inline"
                ...
                <!-- DELETE (only for draft, generated, or rejected) -->
                @if(in_array($record->status, ['draft', 'generated', 'rejected']))
                    <form method="POST" action="{{ route('nfp.destroy', $record->id) }}" class="d-inline"
                          onsubmit="return confirm('⚠️ WARNING: This action cannot be undone.\n\nAre you sure you want to delete this NFP return?')">
                        @csrf
                        @method('DELETE')
    ...
    <!-- INFRASTRUCTURE SUMMARY (NFP SPECIFIC) -->
    @php
        $infrastructure = is_string($record->infrastructure) ? json_decode($record->infrastructure, true) : $record->infrastructure;
    @endphp
    @if($infrastructure && count($infrastructure) > 0)
    <div class="card shadow-sm mb-3">
        <div class="card-header bg-white">
            <strong><i class="fas fa-tower-broadcast me-1 text-kp-yellow"></i> Infrastructure Deployed</strong>
        </div>
        <div class="card-body">
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <strong><i class="fas fa-database me-1 text-kp-yellow"></i> Captured Form Data</strong>
                        ...
                        <table class="table table-sm table-bordered">
                            ...
                            <tbody>
                                @foreach($formData as $key => $value)
                                    @if(!in_array($key, ['_token', '_method', 'save_draft', 'submit', 'infrastructure', 'primary_numbers', 'secondary_numbers', 'bulk_sms', 'broadband', 'staff']))
                                        <tr>
...
{{ json_encode($record->form_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}
    ...
    <!-- ATTACHMENTS SECTION -->
    @php
        $attachments = is_string($record->attachments) ? json_decode($record->attachments, true) : $record->attachments;
    @endphp
                ...
                @foreach($attachments as $name => $path)
                    <div class="col-md-4 mb-2">
                        <div class="border rounded p-2">
                            <i class="fas fa-file-{{ Str::endsWith($path, '.pdf') ? 'pdf text-danger' : 'image text-kp-blue' }} me-2"></i>
                            <strong>{{ ucwords(str_replace('_', ' ', $name)) }}</strong>
                            <br>
                            <small class="text-muted">{{ basename($path) }}</small>
                            <br>
                            <a href="{{ Storage::url($path) }}" target="_blank" class="btn btn-sm btn-outline-kp-primary mt-1">
    ...
    <div class="card shadow-sm mt-3">
        ...
        <div class="card-body">
            ...
            @if($record->cak_reference_number)
                <div class="row mt-2">
                    <div class="col-md-4">
                        <label class="text-muted">CAK Reference Number:</label>
                        <div><code>{{ $record->cak_reference_number }}</code></div>
                    </div>
                </div>
            @endif
        </div>
