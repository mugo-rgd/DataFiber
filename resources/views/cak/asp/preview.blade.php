...
@section('title', 'ASP Compliance Return Preview')
...
<div class="container">
    ...
    <div class="card shadow-sm mb-3">
        <div class="card-body">
            <div class="d-flex flex-wrap gap-2">
                ...
                <!-- PRINT / DOWNLOAD PDF -->
                <a href="{{ route('asp.print', $record->id) }}" target="_blank" class="btn btn-kp-primary">
                    <i class="fas fa-print me-1"></i> Print / Download PDF
                </a>

                <!-- GENERATE PDF (if generated status) -->
                @if($record->status === 'generated')
                    <form method="POST" action="{{ route('asp.generate-pdf', $record->id) }}" class="d-inline">
                        @csrf
                ...
                @if(in_array($record->status, ['draft', 'generated', 'rejected']))
                    <a href="{{ route('asp.edit', $record->id) }}" class="btn btn-kp-warning">
                ...
                @if($record->status === 'draft')
                    <form method="POST" action="{{ route('asp.submit', $record->id) }}" class="d-inline"
                          onsubmit="return confirm('Submit this ASP return for processing?\n\nYou will be able to generate a PDF after submission.')">
                        @csrf
                ...
                @if($record->status === 'submitted')
                    <form method="POST" action="{{ route('asp.submit-to-cak', $record->id) }}" class="d-inline"
                ...
                @if(in_array($record->status, ['draft', 'generated', 'rejected']))
                    <form method="POST" action="{{ route('asp.destroy', $record->id) }}" class="d-inline"
                          onsubmit="return confirm('⚠️ WARNING: This action cannot be undone.\n\nAre you sure you want to delete this ASP return?')">
                        @csrf
                        @method('DELETE')
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <strong><i class="fas fa-database me-1 text-kp-blue"></i> Captured Form Data</strong>
                        ...
                        <table class="table table-sm table-bordered">
                            ...
                            <tbody>
                                @foreach($formData as $key => $value)
                                    @if(!in_array($key, ['_token', '_method', 'save_draft', 'submit']))
                                        <tr>
