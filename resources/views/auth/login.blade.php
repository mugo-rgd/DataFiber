...
<form class="auth-form active" id="login-form" method="POST" action="{{ route('login') }}">
    ...
    @if($lockedOut)
        <div class="alert alert-danger">
            ...
            Please try again in <strong>{{ $lockoutMinutes }} minute(s)</strong>.
        </div>
    @endif
    ...
    @elseif($showAttempts && !$lockedOut && $attempts > 0 && $remainingAttempts > 0)
        <div class="alert alert-kp-warning">
            <i class="fas fa-exclamation-triangle me-2"></i>
            <strong>Login Attempts:</strong> You have {{ $remainingAttempts }} out of {{ $maxAttempts }} attempt(s) remaining.
    ...
    <div class="form-group">
        ...
        <input type="email" name="email" class="form-input @error('email') error @enderror"
               placeholder="Enter your email" required value="{{ old('email') }}" autofocus>
    </div>
   ...
   <div class="form-group">
    ...
    <button type="submit" class="btn btn-kp-primary" id="login-btn">
        <i class="fas fa-sign-in-alt me-2"></i>Login
                ...
                <form class="auth-form" id="customer-signup-form" method="POST" action="{{ route('register.customer') }}">
                    @csrf
